...
# Be aware that carpinchod and carpincho-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in carpinchod.
#
...
# C locale.
#
# In contrast, carpinchod does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where carpinchod is guaranteed to be running
# with the classic locale ("C") whereas the locale of carpincho-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in carpinchod but
# "1,230000" in carpincho-qt.
#
...
KNOWN_VIOLATIONS=(
    "src/carpincho-tx.cpp.*stoul"
    "src/carpincho-tx.cpp.*trim_right"

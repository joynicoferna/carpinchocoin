...
###   carpinchocore.org and carpincho.org and compares them.
...
WORKINGDIR="/tmp/carpincho_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://carpinchocore.org"
HOST2="https://carpincho.org"
BASEDIR="/bin/"
VERSIONPREFIX="carpincho-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'carpincho-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "carpincho.org failed to provide signature file, but carpinchocore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "carpincho.org and carpinchocore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the CARPINCHO Core binary release signing key installed?"
   fi

...
const char * const CARPINCHO_CONF_FILENAME = "carpincho.conf";
const char * const CARPINCHO_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/carpincho/carpincho/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //carpincho-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", CARPINCHO_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "carpincho";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\CARPINCHO
    // macOS: ~/Library/Application Support/CARPINCHO
    // Unix-like: ~/.carpincho
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CARPINCHO";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/CARPINCHO";
#else
    // Unix-like
    return pathRet / ".carpincho";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", CARPINCHO_CONF_FILENAME);

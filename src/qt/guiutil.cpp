...
#include <qt/carpinchoaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a CARPINCHO address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new CARPINCHOAddressEntryValidator(parent));
    widget->setCheckValidator(new CARPINCHOAddressCheckValidator(parent));
}

bool parseCARPINCHOURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no carpincho: URI
    if(!uri.isValid() || uri.scheme() != QString("carpincho"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!CARPINCHOUnits::parse(CARPINCHOUnits::CHO, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseCARPINCHOURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseCARPINCHOURI(uriInstance, out);
}

QString formatCARPINCHOURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("carpincho:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(CARPINCHOUnits::format(CARPINCHOUnits::CHO, info.amount, false, CARPINCHOUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openCARPINCHOConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", CARPINCHO_CONF_FILENAME));
    ...
    /* Open carpincho.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CARPINCHO.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CARPINCHO (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("CARPINCHO (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for CARPINCHO*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "carpincho.desktop";
    return GetAutostartDir() / strprintf("carpincho-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a carpincho.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=CARPINCHO\n";
        else
            optionFile << strprintf("Name=CARPINCHO (%s)\n", chain);

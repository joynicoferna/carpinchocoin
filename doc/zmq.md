...
The CARPINCHO Core daemon can be configured to act as a trusted "border
router", implementing the carpincho wire protocol and relay, making
...
The ZeroMQ feature in CARPINCHO Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building carpinchod:
...
For instance:
    ...
    $ carpinchod -zmqpubhashtx=tcp://127.0.0.1:242005 \
               -zmqpubhashtx=tcp://192.168.1.2:242005 \
               -zmqpubrawtx=ipc:///tmp/carpinchod.tx.raw \
...
These options can also be provided in carpincho.conf.
...
From the perspective of carpinchod, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into carpinchod directly. Furthermore, no information is
...
using. CARPINCHOd appends an up-counting sequence number to each
